...
class Encoder(nn.Module):
	def __init__(self):
		...
		self.encoder = nn.Sequential(nn.Linear(150, 512),nn.PReLU(),nn.Linear(512, 256),nn.PReLU(),nn.Linear(256, 128),nn.PReLU(),nn.Linear(128, 28))
...
class Decoder(nn.Module):
	def __init__(self):
		...
		self.decoder = nn.Sequential(nn.Linear(28, 128),nn.PReLU(),nn.Linear(128, 256),nn.PReLU(),nn.Linear(256, 512),nn.PReLU(),nn.Linear(512, 150))
	def forward(self, x):
		x = self.decoder(x)
		return x
...
def main(args):	
	...
	_, obs, _, _ = load_data(data_path, planner="RRTstar", subset='test')
